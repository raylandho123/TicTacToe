...
males = ExcelResults(1, 1:3); % vector of male biases
females = ExeclResults(1, 6:8); % vector of female biases 

for i = 1:9
    trial_lct = all_means(2, i);
    mean_face = all_means(1, i);
    ...
    while rand_mean ~= tmp_mean
        face = randsample(1:9, num_faces, true);
        rand_mean = mean(face, 'all');
    end
    
    face = face(:, randperm(size(face, 2)));
